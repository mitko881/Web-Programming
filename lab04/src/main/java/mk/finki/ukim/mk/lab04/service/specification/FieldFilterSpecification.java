package mk.finki.ukim.mk.lab04.service.specification;
...
public class FieldFilterSpecification {
    ...
    private static <T> Specification<T> filterEquals(Class<T> clazz, String field, String value) {
    ...
    public static <T, V> Specification<T> filterEqualsT(Class<T> clazz, String field, V value) {
        if (value == null) {
            return null;
        }
        return (root, query, criteriaBuilder) -> criteriaBuilder.equal(fieldToPath(field, root), value);
    ...
    public static <T> Specification<T> filterEquals(Class<T> clazz, String field, T value) {
        if (value == null) {
            return null;
        }
        return (root, query, criteriaBuilder) -> criteriaBuilder.equal(fieldToPath(field, root), value);
    ...
    public static <T> Specification<T> filterContainsText(Class<T> clazz, String field, String value) {
        ...
        return (root, query, criteriaBuilder) ->
                criteriaBuilder.like(criteriaBuilder.lower(fieldToPath(field, (Root<String>) root)), "%" + value.toLowerCase() + "%");
    }
